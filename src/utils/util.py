...
def preallocate_mem(cuda_device, pct=0.98):
    total, used = check_mem(cuda_device)
    total = int(total)
    used = int(used)
    max_mem = int(total * pct)

    block_mem = max(0, max_mem - used)
    x = torch.zeros(256, 1000, block_mem, device=torch.device(f"cuda:{cuda_device}"))
    return x
...
def load_10x_with_meta(data_dir, dataset_dir, sample_id, count_key, annotation_key, filter_genes=1, filter_cells=1, filter_unlabelled=False, load_sce=False):
    if len(sample_id.split('_')) > 1:
        ...
        adata = sc.read_h5ad(osp.join(data_dir, dataset_dir, sample_id, "{}_filtered_feature_bc_matrix.h5".format(sample_id)))
    else:
