import torch
import numpy as np
from src.utils.states import VVI_group, EEE_group
    ...
    def batch_expected_gaussian_loss(self, prob_cat, z_mus, z_logvars, z_mu_priors, z_logvar_priors, affi_weights=None, y_var_invs=None, verbose=False, kind="element-wise"):
      ...
      if not verbose:
         if kind == "element-wise":
            if self.GMM_model_name in VVI_group:
            ...
            elif self.GMM_model_name in EEE_group:
               # B x C -> C x B
            ...
            if self.GMM_model_name in EEE_group:
               raise NotImplementedError
            # C x B x D -> C x D
