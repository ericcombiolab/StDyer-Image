...
import scipy
from scipy.spatial.distance import cdist, minkowski, cosine, euclidean
...
from src.utils.states import GMM_model, Dynamic_neigh_level, VVI_group, EEE_group
...
class GMVGAT(pl.LightningModule):
    ...
    Args:
        ...
        img_dim (int): the output dimension of the image encoder.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.must_link_type = must_link_type
        self.cut_link_loss = cut_link_loss
    ...
    def setup(self, stage):
        ...
        if self.setup_num == 1:
            ...
            self.use_image = self.trainer.datamodule.data_val.use_image
            ...
            assert (self.use_image is False) or (self.img_dim is None)
    ...
    def dynamic_loss(self, x, out_net, output_nodes_idx, input_nodes, output_nodes, rec_neigh_attr, exp_rec_mask=None):
        ...
        link_loss_dict = {"must_link": torch.tensor(0.)}
        ...
        out_net_keys = list(out_net.keys())
        for k in out_net_keys:
            if k not in ["means", "prob_cat"]:
                del out_net[k]
        x_rec = {}
        ...
        del mus, logvars, y_mus, y_logvars, y_var_invs
        if self.inbalance_weight:
        ...
        if self.rec_neigh_num:
            if (self.exp_neigh_w != 0.) or (self.exp_neigh_w_rec != 0. or self.exp_neigh_w_gauss != 0. or self.exp_neigh_w_cat != 0.):
                ...
                if self.dynamic_neigh_level == Dynamic_neigh_level.domain:
                    ...
                    for dynamic_neigh_num in np.unique(dynamic_neigh_nums_arr):
                        ...
                        del tmp_neigh_loss_gauss_dict, neigh_mus, neigh_logvars
                        neigh_loss_gauss.append(tmp_neigh_loss_gauss)
                elif self.dynamic_neigh_level == Dynamic_neigh_level.unit_fix_domain or self.dynamic_neigh_level == Dynamic_neigh_level.unit_fix_domain_boundary or self.dynamic_neigh_level == Dynamic_neigh_level.unit_domain_boundary or self.dynamic_neigh_level.name.startswith("unit_img"):
                    ...
                    neigh_prob_cat = mix_prob_cat[batch_dynamic_neigh_idx]
                    neigh_y_mus, neigh_y_logvars, neigh_y_var_invs = mix_y_mus, mix_y_logvars, mix_y_var_invs
                    del mix_y_mus, mix_y_logvars, mix_y_var_invs
                    ...
                    if self.rec_neigh_num:
                        if self.weighted_neigh:
                            ...
                            neigh_affi_list = 1.
                    del this_cluster_rec_neigh_attr, ref_attr, rec_neigh_attr
                    neigh_prob_cat_list.append(neigh_prob_cat)
                    # for dic_k, dic_v in neigh_x_rec.items():
                    #     print(dic_k, dic_v.shape)
                    gc.collect()
                    ...
                    del x, neigh_x_rec, exp_rec_mask
                    tmp_neigh_loss_rec = tmp_neigh_loss_rec_dict["loss"].reshape(-1, used_k.shape[1]).sum(-1) * self.exp_neigh_ws[output_nodes_idx]
                    del tmp_neigh_loss_rec_dict
                    neigh_loss_rec.append(tmp_neigh_loss_rec)
                    del tmp_neigh_loss_rec
                    neigh_mus, neigh_logvars = mix_mus[:, batch_dynamic_neigh_idx, :], mix_logvars[:, batch_dynamic_neigh_idx, :]
                    del mix_mus, mix_logvars
                    tmp_neigh_loss_gauss_dict = self.losses.batch_expected_gaussian_loss(neigh_prob_cat, neigh_mus, neigh_logvars, neigh_y_mus, neigh_y_logvars, affi_weights=neigh_affi, y_var_invs=neigh_y_var_invs, verbose=self.verbose_loss, kind=self.gaussian_kind)
                    tmp_neigh_loss_gauss = tmp_neigh_loss_gauss_dict["loss"].reshape(-1, used_k.shape[1]).sum(-1) * self.exp_neigh_ws[output_nodes_idx]
                    del tmp_neigh_loss_gauss_dict, neigh_mus, neigh_logvars, neigh_y_mus, neigh_y_logvars, neigh_affi, neigh_y_var_invs, batch_dynamic_neigh_idx
                    neigh_loss_gauss.append(tmp_neigh_loss_gauss)
                    del tmp_neigh_loss_gauss
                ...
                del neigh_affi_list, neigh_prob_cat_list
            ...
            if self.must_link == 0:
                pass
            elif not self.separate_backward:

                if self.must_link_type == "spatial":
                    used_k = self.trainer.datamodule.data_val.adata[data_index].obsm["sp_k"][:, :self.trainer.datamodule.data_val.unit_fix_num]
                    # dynamic neigh idx in original adata: flatten(B x K)
                    dynamic_neigh_idx = used_k.flatten()
                    # dynamic neigh idx in batch: flatten(B x K)
                    if not self.exceed_int_max:
                    ...
                    if self.must_link and self.must_link_type == "spatial":
                        neigh_prob_cat = mix_prob_cat[batch_dynamic_neigh_idx]
                        must_link_loss = self.must_link_loss(prob_cat.repeat_interleave(used_k.shape[1], dim=0), neigh_prob_cat)
                # tic = time.time()

                if self.must_link_type == "image":
                    # data_index are unique indices
                    # input_nodes are expanded unique indices
                    sorted_input_nodes_np = np.sort(input_nodes.cpu().numpy())
                    # input_nodes_np = np.sort(input_nodes.cpu().numpy())
                    if self.must_link and self.must_link_type == "image":
                        if self.device.type == "cpu":
                            img_neigh = NearestNeighbors(n_neighbors=self.trainer.datamodule.data_val.unit_fix_num, n_jobs=-1,
                                                        metric=self.trainer.datamodule.data_val.img_embed_metric)
                            img_neigh.fit(self.trainer.datamodule.data_val.adata[sorted_input_nodes_np].obsm['img_embed'])
                            # img_neigh.fit(self.trainer.datamodule.data_val.adata[input_nodes_np].obsm['img_embed'])
                            img_k = img_neigh.kneighbors(return_distance=False)
                        else:
                            try:
                                img_neigh = cuml.neighbors.NearestNeighbors(n_neighbors=self.trainer.datamodule.data_val.unit_fix_num,
                                                            metric=self.trainer.datamodule.data_val.img_embed_metric,
                                                            output_type="numpy")
                                with cp.cuda.Device(self.device.index):
                                    img_neigh.fit(cp.asarray(self.trainer.datamodule.data_val.adata[sorted_input_nodes_np].obsm['img_embed']))
                                    # img_neigh.fit(cp.asarray(self.trainer.datamodule.data_val.adata[input_nodes_np].obsm['img_embed']))
                                img_k = img_neigh.kneighbors(return_distance=False)
                            except:
                                img_neigh = NearestNeighbors(n_neighbors=self.trainer.datamodule.data_val.unit_fix_num, n_jobs=-1,
                                                            metric=self.trainer.datamodule.data_val.img_embed_metric)
                                ...
                                img_k = img_neigh.kneighbors(return_distance=False)
                        del img_neigh
                        output_node_in_input_node = np.searchsorted(sorted_input_nodes_np, data_index)
                        # print("img_k", img_k.shape)
                        # print("img_k[output_node_in_input_node]", img_k[output_node_in_input_node].shape)
                        img_k = sorted_input_nodes_np[img_k[output_node_in_input_node]]
                        # print("input_nodes", input_nodes.shape)
                        # print("img_k", img_k.shape)
                        # img_k = input_nodes_np[img_k[output_nodes_idx]]
                        # flatten(B x K)
                        dynamic_neigh_idx = img_k.flatten()
                        # # dynamic neigh idx in batch: flatten(B x K)
                        # batch_dynamic_neigh_idx = torch.tensor(img_k[output_nodes_idx.cpu().numpy()]).to(input_nodes).flatten()
                        if not self.exceed_int_max:
                            try:
                                batch_dynamic_neigh_idx = torch.nonzero(input_nodes == torch.from_numpy(dynamic_neigh_idx).to(input_nodes).unsqueeze(1))[:, 1] #.reshape(-1, dynamic_neigh_num)
                                print("batch_dynamic_neigh_idx", batch_dynamic_neigh_idx.shape)
                                raise
                            except RuntimeError:
                                self.exceed_int_max = True
                        if self.exceed_int_max:
                            ...
                            batch_dynamic_neigh_idx = torch.cat(batch_dynamic_neigh_idx)
                        neigh_prob_cat = mix_prob_cat[batch_dynamic_neigh_idx]
                        del batch_dynamic_neigh_idx, mix_prob_cat
                        must_link_loss = self.must_link_loss(prob_cat.repeat_interleave(self.trainer.datamodule.data_val.unit_fix_num, dim=0), neigh_prob_cat)
                if self.must_link:
                    link_loss_dict["must_link"] = must_link_loss * float(self.must_link)
        ...
        return loss_dict, neigh_loss_dict, link_loss_dict
        ...
        if self.force_add_prob_cat_domain is not None:
            ...
            if self.prior_generator.startswith("tensor"):
                for force_domain in self.force_add_prob_cat_domain:
                    ...
                    if self.EXPGMGAT.decoder.GMM_model_name in VVI_group:
                    ...
                    elif self.EXPGMGAT.decoder.GMM_model_name in EEE_group:
    ...
    def training_step(self, batch, batch_idx):
        ...
        exp_loss_dict, neighbor_loss_dict, link_loss_dict = self.dynamic_loss(exp_rec_attr, exp_output_dict, output_nodes_idx, input_nodes, output_nodes, rec_neigh_attr, exp_rec_mask)
        ...
        exp_link_loss = link_loss_dict["must_link"]
        ...
        if not self.separate_backward:
            if self.gaussian_start_epoch_pct == 0.:
                exp_loss = exp_reconstruction_loss + exp_gaussian_loss + exp_categorical_loss + exp_link_loss
            else:
                if self.current_epoch < self.gaussian_start_epoch_pct * self.trainer.max_epochs:
                    ...
                    if self.is_init_cluster_ext():
                        ...
                        exp_loss = self.exp_sup_w_rec * exp_reconstruction_loss + self.exp_sup_w_gauss * exp_gaussian_loss + \
                            self.exp_sup_w_cat * exp_categorical_loss + cluster_init_loss + exp_link_loss
        ...
        if self.separate_backward:
            ...
            if self.must_link:
                if not self.forward_neigh_num:
                    ...
                    exp_output_dict = self.get_forward_outputs(input_block)
                separate_exp_loss_dict, separate_link_loss_dict = self.separate_loss(exp_rec_attr, exp_output_dict, output_nodes_idx, input_nodes, output_nodes, exp_rec_mask,
                                                use_rec_loss=True, use_gauss_loss=False, use_cat_loss=True, use_must_link_loss=True, use_cannot_link_loss=False)
                                                # use_rec_loss=False, use_gauss_loss=False, use_cat_loss=False, use_must_link_loss=True, use_cannot_link_loss=False)
                                                # use_rec_loss=True, use_gauss_loss=True, use_cat_loss=False, use_must_link_loss=True, use_cannot_link_loss=False)
                                                # use_rec_loss=True, use_gauss_loss=False, use_cat_loss=False, use_must_link_loss=True, use_cannot_link_loss=False)
                must_link_loss = separate_link_loss_dict["must_link"]
                must_stage_loss = self.exp_self_w * separate_exp_loss_dict["reconstruction"] + self.exp_self_w * separate_exp_loss_dict["gaussian"] + must_link_loss
                # print("must_stage_loss", must_stage_loss)
                exp_opt.zero_grad()
                self.manual_backward(must_stage_loss)
                exp_opt.step()


            exp_link_loss = must_link_loss
            sch.step()
        ...
        self.training_step_outputs["exp_must_link_loss"].append(link_loss_dict["must_link"].detach().cpu())
        self.training_step_outputs["exp_link_loss"].append(exp_link_loss.detach().cpu())
    ...
    def on_train_epoch_end(self):
        ...
        exp_must_link_loss = torch.mean(self.training_step_outputs["exp_must_link_loss"])
        exp_link_loss = torch.mean(self.training_step_outputs["exp_link_loss"])
        ...
        self.log("train/exp_must_link_loss", exp_must_link_loss, on_step=False, on_epoch=True, prog_bar=False, batch_size=self.curr_batch_size, sync_dist=self.sync_dist)
        self.log("train/exp_link_loss", exp_link_loss, on_step=False, on_epoch=True, prog_bar=False, batch_size=self.curr_batch_size, sync_dist=self.sync_dist)
        ...
        if self.current_epoch == 0 and self.prior_generator == "tensor_kmpp":
            self.training_step_outputs["epoch_exp_means"]
        ...
        if self.last_epoch_exp_pred_df is not None:
            ...
            if self.current_epoch >= self.patience_start_epoch_pct * self.trainer.max_epochs - 1: # [0, 49], 50~, data preparation starts at the end of the 49th epoch
                ...
                if self.dynamic_neigh_level == Dynamic_neigh_level.domain:
                    ...
                    self.trainer.datamodule.data_val.adata.obsm["consist_adj"][data_index] = adata_ref.obs["pred_labels"].to_numpy()[adata_ref[data_index].obsm["sp_k"]] == each_obs_neighbor_cat[:, np.newaxis]
                    self.trainer.datamodule.data_val.dynamic_neigh_nums = np.minimum(self.max_dynamic_neigh, self.trainer.datamodule.data_val.adata.obsm["consist_adj"].sum(1))
                elif self.dynamic_neigh_level.name.startswith("unit_fix_domain") or self.dynamic_neigh_level.name.startswith("unit_img"):
    ...
    def on_validation_epoch_end(self):
        ...
        if self.trainer.is_global_zero:
            ...
            if len(predicted_domains) < self.num_classes:
                ...
                if ((self.current_epoch >= self.trainer.max_epochs) or \
                    (self.not_enough_classes == 3)) and \
                        (not self.connect_prior) and \
                        (self.prior_lr != 0.) and \
                        (self.force_enough_classes is False) and \
                        (self.trainer.datamodule.resample_to is None) and \
                        (self.current_epoch >= self.gaussian_start_epoch_pct * self.trainer.max_epochs + self.sup_epochs) and \
                        ((len(predicted_domains) == 1) or \
                            (self.num_classes - len(predicted_domains) >= 2) or \
                            ((self.num_classes - len(predicted_domains) == 1) and \
                                (len(self.last_normal_epoch_dict.keys()) > 0) and \
                                (self.current_epoch - list(self.last_normal_epoch_dict.keys())[-1] >= 20))):
    ...
    def must_link_loss(self, repeat_prob_cat, neigh_prob_cat):
        consistency = repeat_prob_cat.argmax(1) == neigh_prob_cat.argmax(1)
        if self.cut_link_loss:
            link_loss = -torch.log((repeat_prob_cat * neigh_prob_cat)[consistency].sum(1))
            return link_loss[torch.isfinite(link_loss)].mean()
        link_loss = -torch.log((repeat_prob_cat * neigh_prob_cat).sum(1))
        return link_loss[torch.isfinite(link_loss)].mean()
